  ...
  return (
    <div>
      <PageHeader 
        ...
        description="Let AI build the perfect outfit from your wardrobe."
      />

      {/* Input Form */}
      <Card className="mb-8">
        <CardContent className="p-6">
          <form onSubmit={generateOutfit} className="flex flex-col sm:flex-row gap-4 items-end">
            <Input 
              label="Occasion"
            ...
            />
            <Button type="submit" className="w-full sm:w-auto shrink-0" isLoading={loading}>
      ...
      {loading && (
        <div className="py-20 flex flex-col items-center">
          ...
          <p className="text-sm text-cloudburst">Analyzing your wardrobe combinations...</p>
        </div>
      )}
      ...
      {recommendation && (
        <div className="space-y-6">
          {recommendation.insufficient_wardrobe && (
            <div className="bg-code-orange/8 border border-code-orange/20 p-4 rounded-xl flex gap-3 text-code-orange">
              <TriangleAlert className="h-5 w-5 shrink-0 mt-0.5" />
              <div>
                <p className="text-sm font-medium">Insufficient Wardrobe</p>
                <p className="text-xs opacity-80 mt-0.5">You don&apos;t have enough items to build a complete outfit. Showing partial results.</p>
              </div>
            </div>
          )}
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                    <Sparkles className="h-4 w-4 text-cyber-cyan" /> Stylist Reasoning
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <p className="text-sm text-cloudburst leading-relaxed">{recommendation.explanation}</p>
                </CardContent>
              </Card>
            </div>
...
function OutfitSlot({ title, item }: { title: string, item?: RecommendedItem }) {
  return (
    <div>
      <p className="text-[11px] font-medium text-cloudburst mb-2 uppercase tracking-wider">{title}</p>
      <div className="aspect-[3/4] bg-surface-raised rounded-xl border border-border-subtle overflow-hidden flex items-center justify-center relative">
        {item ? (
        ...
        {item && (
          <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/80 to-transparent p-2.5">
            <p className="text-[11px] text-porcelain truncate font-medium">{item.type}</p>
          </div>
        )}
      </div>
    </div>
